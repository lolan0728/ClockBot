...
function Clear-Target {
  ...
  if (Test-Path -LiteralPath $fullPath) {
    Write-Status ('Still exists after cleanup attempts: ' + $fullPath + '. Run this script from Windows Safe Mode for the highest chance of removal.')
    return $false
  }
...
$logPath = Join-Path $env:USERPROFILE 'Desktop\\ClockBot_Cleanup_Log.txt'
...
try {
  ...
  foreach ($target in $targets) {
    $ok = Clear-Target -Path $target -ClockBotRoot $clockBotRoot -EmptyDir $emptyDir
    if (-not $ok) {
  ...
  if ($failedTargets.Count -gt 0) {
    Write-Status 'Cleanup finished with remaining paths:'
    foreach ($target in $failedTargets) {
      Write-Status ('Remaining: ' + $target)
    }
    throw 'One or more failed profile directories still remain. Run this script from Windows Safe Mode and try again.'
  }
